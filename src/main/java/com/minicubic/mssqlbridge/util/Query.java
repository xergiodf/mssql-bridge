...
public class Query {
    ...
    public static final String INSERT_HISTORICO_SQL = "insert into historicos (id_gestion, id_usuario, id_estado, id_resultado, id_motivo, id_tipo_llamada, comentario) select id as id_gestion, ? as id_usuario, id_estado, id_resultado, id_motivo, 3 as id_tipo_llamada, UPPER(?) as comentario from gestiones where id = ? and id = ? and id = ? and id = ? and id = ?";
    
    public static final String getListSQLHojaRuta(Integer userId) {
        return "select distinct hr.id as id, hr.descripcion as descripcion, hr.fecha_impresion, z.zona from gestiones g join materiales m on ( g.id_material = m.id ) join estados_gestiones eg on ( g.id_estado = eg.id ) join gestiones_hojas_de_ruta ghr on ( g.id = ghr.id_gestion ) join hojas_de_ruta hr on ( ghr.id_hoja_ruta = hr.id ) join zonas z on ( hr.id_zona = z.id ) join usuarios u on ( hr.id_usuario_mensajero = u.id ) where g.id_resultado = 6 and u.id = " + userId + " and g.id_estado = 4 order by hr.fecha_impresion desc; ";
    }
